# Changelog

## [0.5.0] - 2026-06-22

### Added

- 新增记录模块，支持每日工资自动记录、历史查询、手动补录与修正，以及 CSV / JSON 导出。
- 新增月度收入、工时和工作日目标，支持默认目标、进度状态和预计月底收入。
- 新增统计模块，展示月度收入与工时汇总、最近 7 天趋势及异常记录提示。
- 引入本地 SQLite 业务数据存储，并保留 JSON 应用配置存储。

### Changed

- 统一 Windows 安装包与自动更新元数据的文件名格式。

## [0.4.2] - 2026-06-02

### Changed

- Replaced remaining SVG/CSS-drawn control icons with generated metallic PNG assets for settings controls, window controls, side navigation, and the top drag indicator.
- Removed the unused `lucide-react` dependency after moving controls to project-local image assets.

## [0.4.0] - 2026-06-02

### Added

- 增加 GitHub Releases 自动更新检测能力，支持启动后延迟检查、设置面板/托盘手动检查、下载进度展示和重启安装。

## [0.3.0] - 2026-05-27

### Changed

- 全面重构主界面视觉，从旧版蓝紫卡片风格切换为黑金铜版画质感，并同步提供浅色纸张铜色主题。
- 将底部功能区迁移为右侧折叠工具栏，降低窗口纵向占用并保留隐藏金额、统计、目标、记录和设置入口。
- 调整默认窗口尺寸、顶部日期天气布局、进度条、外框、按钮图标和设置面板细节，使界面更贴近新设计稿。

### Fixed

- 修复主题切换后浅色主题不明显的问题，切换时现在会即时更新渲染主题。
- 修复侧边栏折叠状态图标偏移、日期星期换行、窗口圆角残影、天气温度错位等 UI 对齐问题。

## [0.2.6] - 2026-05-26

### Fixed

- 将顶部最小化按钮调整为收起到最近屏幕边缘，避免与置顶防收起行为冲突。

## [0.2.5] - 2026-05-26

### Fixed

- 隐藏应用桌面任务栏入口，保持托盘小组件形态。
- 修复窗口置顶时仍会被 Win+D 收起的问题。

本项目采用 [Semantic Versioning](https://semver.org/lang/zh-CN/) 版本号规范：`MAJOR.MINOR.PATCH`。

## [0.2.4] - 2026-05-19

### Changed

- 将底部固钉按钮功能从靠边收起调整为切换窗口置顶。
- 固钉按钮新增置顶激活态，便于识别当前窗口置顶状态。

## [0.2.3] - 2026-05-19

### Changed

- 靠边收起提示条进一步收细，默认宽度降为 `4px`。
- 优化靠边吸附、收起和展开速度，减少窗口定位动画延迟，提升跟手感。

## [0.2.2] - 2026-05-19

### Changed

- 靠边收起提示条改为单一色并进一步收细，改善上下边缘收起时的观感。
- 降低浅色主题天气卡片和本月数据卡片的背景 tint 强度。

## [0.2.1] - 2026-05-19

### Changed

- 将靠边吸附整合进靠边收起开关，开启靠边收起时自动吸附，关闭时不吸附。
- 移除设置面板中的独立 `靠边吸附` 开关，减少设置项高度。

## [0.2.0] - 2026-05-19

### Added

- 新增靠边吸附功能，窗口拖动接近屏幕边缘时会自动贴齐边缘。
- 靠边吸附与靠边收起联动：同时开启时，吸附边缘会作为后续收起边缘。
- 设置面板新增 `靠边吸附` 开关，可与 `靠边收起` 独立控制。

## [0.1.10] - 2026-05-19

### Changed

- 将本月数据卡片的淡色主题渐变同步应用到天气卡片。
- 调整深色和浅色主题下的卡片背景层次，使内部卡片更柔和。

## [0.1.9] - 2026-05-19

### Changed

- 优化本月数据模块布局，将标题移动到图标同一行并压缩卡片高度。
- 为本月数据卡片增加淡色主题背景，减少浅色模式下的单调感。

## [0.1.8] - 2026-05-19

### Changed

- 靠边收起提示条移除内部文字，改为纯胶囊色块。
- 调整浅色和深色主题的进度条、靠边提示条渐变配色，使整体更现代、轻盈。

## [0.1.7] - 2026-05-19

### Fixed

- 靠边收起提示条改为短胶囊形状，减少桌面视觉遮挡。
- 修复鼠标悬停展开后仍被自动收起的问题。

## [0.1.6] - 2026-05-19

### Fixed

- 时间选择器隐藏原生系统滚动条，改为自绘细圆角滚动条，避免 Windows 方块滚动条残留。

## [0.1.5] - 2026-05-19

### Fixed

- 进一步优化时间选择器滚动条，隐藏原生滚动按钮并收窄为圆润细条。
- 统一薪资模式、主题、时间选择控件的小箭头位置。

## [0.1.4] - 2026-05-19

### Changed

- 优化日薪 / 月薪数字输入框样式，移除原生计数器。
- 优化时间选择器滚动条样式，改为更细、更圆润的控件内滚动条。

## [0.1.3] - 2026-05-19

### Changed

- 将设置面板中的薪资模式和主题模式改为自绘下拉控件，避免 Windows 原生下拉样式割裂。
- 将工作时间和午休时间改为自绘双列时间选择器，统一深浅主题、圆角、悬停态和选中态。

## [0.1.2] - 2026-05-19

### Changed

- 主题切换选项文案调整为 `跟随系统`、`深色`、`浅色`。
- 优化设置面板下拉框样式，增加自定义箭头、悬停态和聚焦态。

## [0.1.1] - 2026-05-19

### Fixed

- 去除透明窗口四角外扩阴影，避免圆角区域残留直角阴影。
- 调整桌面卡片为更紧凑的流式布局，底部状态栏不再覆盖本月统计内容。
- 增加主窗口高度，确保默认信息在小组件内完整显示。

## [0.1.0] - 2026-05-19

### Added

- 初始化 Electron + Vite + React 桌面应用。
- 新增桌面工薪小卡片主界面。
- 新增当前日期时间、今日已赚工薪、今日工时进度、本月已赚、本月已过天数展示。
- 新增日薪 / 月薪配置。
- 新增工作时间、午休时间、窗口透明度配置。
- 新增明亮 / 黑暗 / 跟随系统主题切换。
- 新增开机自启动、窗口置顶、隐私金额配置。
- 新增靠边收起和提示条唤回能力。
- 新增应用内版本号展示，版本来源为 `package.json`。
